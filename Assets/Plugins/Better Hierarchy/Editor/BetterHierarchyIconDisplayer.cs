    ...
    public static class BetterHierarchyIconDisplayer
    {
        ...
        private static void OnEditorUpdate()
        {
            if (!hierarchyEditorWindow && IsHierarchyWindowFocused())
            {
                hierarchyEditorWindow = EditorWindow.GetWindow(
                    Type.GetType(
                        $"{nameof(UnityEditor)}.SceneHierarchyWindow,{nameof(UnityEditor)}"
                    )
                );
            }

            hierarchyHasFocus =
                EditorWindow.focusedWindow && EditorWindow.focusedWindow == hierarchyEditorWindow;
        ...
        private static void OnHierarchyWindowItemOnGUI(int instanceID, Rect selectionRect)
        {
            ...
            HierarchyObjectStatus SetupHierarchyObjectStatus()
            {
                ...
                objectStatus.IsDropDownHovered = expandChildrenIconRect.Contains(
            ...
            {
                if (
                    objectStatus.IsSelected
                    || (objectStatus.IsDropDownHovered && MouseStatus.IsMouseDown)
                )
                {
            ...
            {
                return (
                    IsOverridePrefabIcons
                    && OverridenPrefabIcons == ScriptIconType.UnityDefault
                    && IsPrefab(obj)
                );
            }
        }
        ...
        )
        {
            ...
            Color originalColor = GUI.color;
            {
                ...
                {
                    Color transparentIconColor = new Color(
                        originalColor.r,
                        originalColor.g,
                        originalColor.b,
                        0.5f
        ...
        private static void ClearOriginalIcon(
            HierarchyObjectStatus hierarchyObjectStatus,
            Rect selectionRect
        )
        {
            int selectedAmount =
            ...
            Color color = UnityEditorBackgroundColor.Get(
                hierarchyObjectStatus,
                hierarchyHasFocus,
                selectedAmount
            );
        ...
        private static bool HasCustomScripts(
            IReadOnlyList<Component> components,
            out Component customComponent
        )
        {
            ...
            int componentsToCheckAmount = Mathf.Min(
                componentsCount,
                MAX_COMPONENT_CHECKS + FIRST_COMPONENT_INDEX
            );
        ...
        private static bool IsNamespaceUnityRelated(Component component)
        {
            ...
            if (UnityScriptDetectionType == UnityNativeScriptsDetectionType.Unity)
                ...
                isNativeUnityRelated =
                    namespaceStr.Equals(nameof(UnityEngine))
